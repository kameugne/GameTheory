...
public class GenerateCompleteGame {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                if (i != j) {
                    ...
                    new Constraint("monotony", new PropertyConstraint(subSet.get(i), subSet.get(j), value[i], value[j],n)).post();
                }
            }
        }
        new Constraint("Completude", new CompletudeConstraint(subSet, value, n)).post();
    ...
    public static boolean Include(SetVar X, SetVar Y) {
        ISetIterator it = X.getUB().iterator();
        while (it.hasNext()) {
            int x = it.next();
            if (!Y.getLB().contains(x))
                return false;
        }
        return true;
    }
